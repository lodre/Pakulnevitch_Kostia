...
let ts_rev = Array.make (Array.length a) (-1);;

for i = 0 to Array.length a - 1 do
    ts_rev.(ts.(i))<-i
done;; 

let ways = Array.make (Array.length a) (-1);;
ways.(0)<-1;;
let rec ways_to i j = try
                          if a.(j).(i) = 1 then ways.(j) + (ways_to i (j+1))
                                           else ways_to i (j+1)
                      with _->0;;
for i = 1 to Array.length a - 1 do
    ways.(ts.(i)) <- ways_to (ts.(i)) 0
done;;

print_int (ways.(Array.length a - 1));;